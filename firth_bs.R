firth_bs<-function(beta, tau, D,X,threshold_lik=10^-10,M_iter=10,maxstep=5,half_iter=5, printLik  = FALSE){
  ...
  # gamma<-array(0,c(n,n,Q,Q))	# This is the term tau_{iq}tau_{jq} if q=l and tau_{iq}tau_{jl}+tau_{il}tau{jq} if q<l
  ...
  # for(l in q:Q){
  # if(q==l){
  # gamma[,,q,l]=tcrossprod(tau[,q])                  #outer product between the same block tau vectors
  # }else{
  # gamma[,,q,l]=tcrossprod(tau[,q],tau[,l])        # outer product between the different  block tau vectors
  # gamma[,,q,l]=gamma[,,q,l]+t(gamma[,,q,l])  # populating lower matrix part diagonal ignored  
  # }
  # sum_gamma_without_diag[q,l] = sum(gamma[,,q,l])-sum(diag(gamma[,,q,l])) # summation on gamma remove diag
  # for (k in 1:K){
  # tmp <- gamma[,,q,l]
  # sum_connection[q,l,k] <- sum(tmp[X[,,k]==1])   # summation on gamma when xijk=1
  # sum_no_connection[q,l,k] <-  sum_gamma_without_diag[q,l] - sum_connection[q,l,k]
  ...
  while(m <=M_iter & delta_lik>threshold_lik){
    ...
    # lik_previous=lik_beta			       					    				# lik_beta not known at the first iteration (m=1)
      ...
      for(l in q:Q){
        ...
        # } else {	
          ...
          if(any(abs(delta_beta[q,l,])> maxstep)){
            delta_beta[q,l,abs(delta_beta[q,l,])> maxstep] = maxstep * sign(delta_beta[q,l,abs(delta_beta[q,l,])> maxstep])
    ...
    while (ihalf_iter < half_iter & any(b_id)){
      ...
      b_id <- (block_lik_beta_star-block_lik_beta_star_previous)<0 
